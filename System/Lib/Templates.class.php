...
class Templates{
	...
	public function display($_file,$_path=FALSE){
		...
		if($_path){//适配内置模板 success,error
		    ...
		    if(is_null($_file)){
			   ...
			   $this->_tplFile.=$_controller.'/'.strtolower($_action).'.'.C('TEMPLATE_EXT');
			   $this->_parseFile.=$_controller.'/'.md5($_action.'.'.C('TEMPLATE_EXT')).'.'.strtolower($_action).'.'.C('TEMPLATE_EXT').'.php';
			   $this->_cacheFile.=$_controller.'/'.md5($_action.'.'.C('TEMPLATE_EXT')).'.'.strtolower($_action).'.'.C('TEMPLATE_EXT').'.html';//设置缓存文件路径
			
		    }elseif(strpos($_file,'/')){ // 跨模块调用模版文件
			   list($_controller,$_file)  =   explode('/',$_file);
			   $_controller=ucfirst($_controller);
			   //如果没有后缀  加上配置后缀
			   $_file=$this->is_ext($_file);
	...
	public function is_ext($_file){
		if(!strpos($_file,'.')){
			$_file.='.'.C('TEMPLATE_EXT');
		}
